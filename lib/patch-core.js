 ...
 *
 * 另：PATCHED_FN 里 alnum() 用 `b % alphabet.length` 生成随机字母数字段
 * （62 字符表对 256 取模有轻微分布偏差）。OpenCode Zen 网关只校验值的正则
 * 格式 `[0-9A-Za-z]{14}`，不校验统计分布，功能无影响；修复需改函数体并升级
 * MARKER（v5），会让已部署的 v4 机器走一次无必要的升级路径，故保留现状。
 */
